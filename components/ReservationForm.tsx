...
return ( <form
   className="form"
   onSubmit={submit}
 > <div className="selected-box"> <b>{formatDate(start)}</b> →{' '} <b>{formatDate(effectiveEnd)}</b>
...
**Con este cambio, al completar el formulario y tocar “CONTINUAR CON LA RESERVA”, ya no va a buscar `/api/reservas`**, por lo que no debería aparecer el error que estabas viendo. Va directamente al cartel de transferencia.

⚠️ **Pero ojo con una cosa:** esto hace funcionar el cartel, pero **todavía no guarda la reserva en el panel de administración**. Ese es un segundo paso. Si queremos que el admin reciba automáticamente nombre, fechas, teléfono, total, etc., vamos a tener que crear el endpoint que hoy no existe.

Primero probá **este cambio**. Si te aparece el cartel de transferencia correctamente, seguimos con el guardado en admin sin tocar lo que ya funciona.
```
